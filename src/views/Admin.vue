    ...
    <div v-if="isLoggedIn" style="margin-top: 30px;">
      ...
      <v-dialog v-model="dialog" width="auto" persistent>
        <v-card width="400">
          ...
          <template #text>
            <v-container>
              <v-text-field label="Old Password" type="password" v-model="passwordForm.oldPassword"></v-text-field>
              <v-text-field label="New Password" type="password" v-model="passwordForm.newPassword"></v-text-field>
              <v-text-field label="Retype New Password" type="password"
                v-model="passwordForm.retypeNewPassword"></v-text-field>
              <v-alert v-if="alertText != ''" type="error">{{ alertText }}</v-alert>
            </v-container>
          </template>
          <template #actions>
            <v-container>
              <v-btn :loading="isUpdatingPassword" @click="updatePassword()">Update</v-btn>
              <v-btn :loading="isUpdatingPassword" @click="dialog = !dialog">Close</v-btn>
            </v-container>
          </template>
        </v-card>
      </v-dialog>
...
const newLinkText = ref(''), alertText = ref('');
